...
Ans). Path variable: It is used to extract values from the URL/URI path where value enclosed with "{}" braces.(
@PathVariable)
...
The CAP theorem emphasizes the importance of identifying trade-offs in distributed system design to meet business needs
effectively.
...
* Q:). Spring Bean scope ?
  In Spring, a bean scope defines the lifecycle and visibility of a bean instance.
  By default, Spring beans are singleton-scoped (@Scope("singleton")), but Spring provides several predefined scopes,
  and you can also create custom scopes.

✅ Common Built-in Scopes in Spring
Scope Description
**singleton**    (Default) A single instance shared across the entire application context.
**prototype** A new instance created every time it is requested from the context.
**request** A new bean instance for every HTTP request (only in web apps).
**session** A new bean instance per HTTP session (only in web apps).
**application** A bean scoped to the lifecycle of the ServletContext.
**websocket** A new bean per WebSocket session (only in WebSocket apps).
