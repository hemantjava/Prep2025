...
public class UniqueLongestSubstring {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Map : " + longestSubstringLengthMap(input));
    }
    ...
    public static int longestSubstringLength(String string) {
        ...
        for (int i = 0; i < string.length(); i++) {
            ...
            if (!map.containsKey(visited)) {
                map.put(visited, i); // value and index store
            } else {
                i = map.get(visited);
                map.clear(); //if visited then clean the map
            }
    ...
    public static Map<String, Integer> longestSubstringLengthMap(String string) {
        ...
        for (int i = 0; i < string.length(); i++) {
            ...
            if (map.size() > maxLength) {
                maxLength = map.size();
                val = map.keySet().stream().map(String::valueOf).collect(Collectors.joining());
            }
        }
        return Map.of(val, maxLength);
    }
